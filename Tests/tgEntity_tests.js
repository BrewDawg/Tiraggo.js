...
test('Testing Multiple Objects prototype chain', function () {
    ...
    //define an object
    var emp1 = new tg.objects.Employees();
    emp1.EmployeeID(42);
    emp1.OtherEmployeeID(43);

    var emp2 = new tg.objects.Employees();
    emp2.EmployeeID(44);
...
test('Testing Multiple Objects prototype BASE chain', function () {
    ...
    var emp1 = new tg.objects.Employees();
    emp1.RowState(tg.RowState.UNCHANGED); //Mark it as unchanged
    ...
    equals(emp1.RowState(), tg.RowState.MODIFIED, 'Base RowState changed independently of other instances');

    equals(emp2.RowState(), tg.RowState.ADDED, 'Base RowState changed independently of other instances');
...
test('IsDirty Test', function () {
    ...
    equals(emp.RowState(), tg.RowState.ADDED, 'RowState has changed to modified');
});
...
test('Ensure Change Tracking Test', function () {
    ...
    equals(emp.RowState(), tg.RowState.ADDED, 'Newly Instantiated Object has RowState of Added');
});
...
test('Ensure Change Tracking Test - Modification', function () {
    ...
    emp.RowState(tg.RowState.UNCHANGED);
    emp.EmployeeID(99);

    equals(emp.ModifiedColumns().length, 1, 'Updated EmployeeID and Modified Columns reflects one field');
    equals(emp.RowState(), tg.RowState.MODIFIED, 'RowState has changed to modified');
});
...
test('Accept Changes - Modification', function () {
    ...
    equals(emp.tg.originalValues['EmployeeID'], undefined, 'Original Values is empty');
    equals(emp.RowState(), tg.RowState.UNCHANGED, 'RowState has been set back');
});
...
test('Reject Changes - Adding', function () {
    ...
    equals(emp.tg.originalValues['EmployeeID'], undefined, 'Original Values is empty');
    equals(emp.RowState(), tg.RowState.ADDED, 'RowState has been set back');
});
...
test('Ensure "populateEntity" works and "ExtraColumns" are flattened', function () {
    ...
    //define an object
    var emp = new tg.objects.Employees();
    emp.populateEntity({ "EmployeeID": 42, "tgExtendedData": [{ "Key": "ExtraColumn", "Value": "asdf" }, { "Key": "IDasExtraColumn", "Value": 1519}] });
...
test('Test Basic Save', function () {
    ...
    //define an object
    var emp = new tg.objects.Employees();
    emp.populateEntity({ "EmployeeID": 42, "ExtendedData": [{ "Key": "ExtraColumn", "Value": "asdf" }, { "Key": "IDasExtraColumn", "Value": 1519}] });
    emp.RowState(tg.RowState.UNCHANGED);
    ...
    equals(saveReq.data.RowState, tg.RowState.MODIFIED, 'Correct RowState was handed Back');
});
